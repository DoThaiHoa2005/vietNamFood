...
namespace VietnamFoodGuide.Services
{
    ...
    public class SQLiteFoodService
    {
            ...
            try
            {
                using (var conn = new SQLiteConnection(_connectionString))
                {
                    ...
                    string[] alterTableSqls = new[]
                    {
                        ...
                        "ALTER TABLE Foods ADD COLUMN AudioUrl_VI TEXT",
                        "ALTER TABLE Foods ADD COLUMN AudioUrl_EN TEXT",
                        "ALTER TABLE Foods ADD COLUMN AudioUrl_CN TEXT",
                    ...
                    foreach (var sql in alterTableSqls)
                    {
                        ...
                        {
                            // It's normal to throw if column already exists
                            System.Diagnostics.Debug.WriteLine($"[SQLiteFoodService] Migration info (column might exist): {ex.Message}");
                        }
                    }
            ...
            using (var conn = new SQLiteConnection(_connectionString))
            {
                ...
                string insertSql = @"
                    INSERT INTO Foods (Name, City, Category, Image, DescriptionVI, DescriptionEN, DescriptionCN, 
                                       Latitude, Longitude, Rating, Radius, Priority, AudioUrl_VI, AudioUrl_EN, AudioUrl_CN, CooldownMinutes)
                ...
                using (var cmd = new SQLiteCommand(insertSql, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@AudioUrl_VI", food.AudioUrl_VI ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@AudioUrl_EN", food.AudioUrl_EN ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@AudioUrl_CN", food.AudioUrl_CN ?? (object)DBNull.Value);
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            foods.Add(new FoodItem
                            {
                                ...
                                AudioUrl_VI = GetStringOrNull(reader, "AudioUrl_VI"),
                                AudioUrl_EN = GetStringOrNull(reader, "AudioUrl_EN"),
                                AudioUrl_CN = GetStringOrNull(reader, "AudioUrl_CN"),
        ...
        /// <summary>
        /// Helper: Đọc cột string nullable an toàn (tránh crash nếu cột chưa có)
        /// </summary>
        private string GetStringOrNull(System.Data.SQLite.SQLiteDataReader reader, string columnName)
        {
            try
            {
