        ...
        private async System.Threading.Tasks.Task ToggleFavoriteAsync()
        {
            try
            {
                ...
                if (App.CurrentApiUser != null)
                {
                    if (_food.Id <= 0)
                    {
                        MessageBox.Show("Món ăn không có ID hợp lệ!", _lang["error"], MessageBoxButton.OK, MessageBoxImage.Error);
                        return;
                    }
                    ...
                    if (_food.IsFavorite)
                    {
                        ...
                        else MessageBox.Show($"{_lang["error"]}: {result.message}", _lang["error"], MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                    else
                    {
                ...
                {
                    MessageBox.Show(_lang["need_login"], _lang["notification"], MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            {
                MessageBox.Show($"{_lang["error"]}: {ex.Message}", _lang["error"], MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
